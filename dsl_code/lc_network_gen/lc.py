...
# added support for multiple files in one run
# writes output to ./comm_data/
# Bobak Hadidi
...
def write_edge2cid(e2c,filename,delimiter="\t",path=''):
    # write edge2cid three-column file
    f = open(path+filename+".edge2comm.txt",'w')
    ...
    f,g = open(path+filename+".comm2edges.txt", 'w'),open(path+filename+".comm2nodes.txt", 'w')
...
if __name__ == '__main__':
    usage = "usage: python %prog [options] filename(s)"
    ...
    if len(args) < 1:
        parser.error("Enter at least one edgelist file")
    ...
    while i != len(args):
        ...
        adj,edges = read_edgelist(args[i], delimiter=delimiter) # load network from edgelist
