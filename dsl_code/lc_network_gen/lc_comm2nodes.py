...
def similarities(adj):
    ...
    if 'saved_sim_comp' in os.listdir('.'):
       f = open('saved_sim_comp','r')
       min_heap = cPickle.load(f, 'r')
       f.close()
       f = open('last_count','r')
       lastcount = int(f.readline().strip())
       f.close()
    else:
    ...
    for n in adj: # n is the shared node
        ...
        if len(adj[n]) > 1:
            try:
                ...
                cPickle.dump(min_heap, open('saved_sim_comp','w'))
                f = open('last_count', 'w')
                f.write(str(count))
                f.close()
                sys.exit()
